...
# 保护快照（必需步骤）
rbd snap protect cinder-pool/volume-0b75ba15-bb6b-40fe-adb9-24dbfd188348@snap01

# 创建克隆卷
rbd clone cinder-pool/volume-0b75ba15-bb6b-40fe-adb9-24dbfd188348@snap01 cinder-pool/k8s-clone
...
- 管理员/网络/网络
    - 创建网络（click）
        ...
        - 子网详情
            - 分配地址池 192.168.200.21,192.168.200.30（随意）
            - dns 服务器 223.5.5.5
### private
- 登陆用户 zhangmingming
- 项目/网络/网络
...
neutron port-update 9374b3e1-92ff-400b-b993-df24162123f1 --allowed_address_pairs list=true type=dict ip_address=172.17.10.238
neutron port-update f597bdf1-0aa6-469c-9d27-c258a45118f6 --allowed_address_pairs list=true type=dict ip_address=172.17.10.238
...
openstack port create --network df9b2572-d204-497e-9b78-c02f7a03c147 --fixed-ip subnet=bc2d4461-69a4-4cef-a459-0de3bfbb0225,ip-address=172.17.10.239 viptest1
...
```shell
# 在master1节点操作
kubeadm init --control-plane-endpoint=192.168.200.239:16443 --image-repository registry.aliyuncs.com/google_containers --kubernetes-version v1.31.7 --service-cidr=10.96.0.0/16 --pod-network-cidr=10.244.0.0/16 --cri-socket unix:///var/run/cri-dockerd.sock 
# 初始化失败，删除
...
scp -r /etc/kubernetes/pki root@k8sm2:/etc/kubernetes/
scp -r /etc/kubernetes/pki root@k8sm3:/etc/kubernetes/
```
### m02\m03 加入集群
```shell
...
scp -rp /etc/kubernetes/pki root@m02:/etc/kubernetes/
scp -rp /etc/kubernetes/admin.conf root@m02:/etc/kubernetes/

scp -rp /etc/kubernetes/pki root@m03:/etc/kubernetes/
