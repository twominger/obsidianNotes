...
| 节点         | 网络                   | CPU | 内存  | 硬盘  |
| ---------- | -------------------- | --- | --- | --- |
| controller | 192.168.224.100 (桥接) | 4   | 8   |     |
| compute1   | 192.168.224.101      | 4   | 10  |     |
| compute2   | 192.168.224.102      | 4   | 8   |     |
...
## 部署流程
- vmware 虚拟机准备，两台，NAT，除系统盘外再加 9 块硬盘
- 安装必要组件
```shell
...
```
2.6 运维工程师为了调整云硬盘中操作系统的数据，决定对 cinder 中的 k8s-image 镜像进行改造，使用 rbd 克隆技术得到克隆卷 k8s-clone 并将克隆的镜像挂载起来，删除镜像中 k8s 的配置文件，然后将其导出到，灾备站点的 backup-pool 的存储池中，命名为 k8s-image-backup
```shell
# 创建快照
