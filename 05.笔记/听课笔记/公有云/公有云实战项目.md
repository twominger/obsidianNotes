一、项目背景介绍 
公司 A 计划使用华为云部署在线电子商务系统，请根据如下需求，完成搭建
1 . 公司位于北京市，选择北京四部署业务站点。 
2 . 考虑架构设计的可靠性，前端业务主机和后端数据库均需要跨可用区主备的容灾环境。 
3 . 使用 windows 进行主备容灾测试。 
4 . 为了防止数据丢失，需要做前端云主机和数据库进行备份 
5 . 公司原有业务需要迁移到华为云，全面云化。 
6 . 维护系统安全，需要将前端资源和后端数据库进行安全隔离 
二、项目需求 
7. 在北京四区部署 opencart 电子商务系统，ECS 主机规格为 2vCPUs|4GiB，镜像为公共镜像：Centos7.6 64 
8. 为了保证 opencart 电子商务系统业务连续性，要求实现跨可用区容灾，并进行故障测试。 
9. 数据库使用云数据库 RDS，数据库引擎 5.7，要求主备部署，与 opencart 电子商务系统可用分区保持一致；并实现读写分离，创建两个只读库，两个只读库读权重为 5，主库读权重为 1 注意：opencart 需要连接读写分离后代理数据库地址 
10. 基于安全考虑，opencart 电子商务系统和后端 MySQL 数据库需要使用不同的 VPC 和安全组，安全组开放最小原则。 
11. 为应对未来 opencart 高并发请求，要求使用 ELB 来提供负载均衡，使用加权轮询算法，后端需要健康检查，并进行负载均衡测试。 
12. 由于该电商平台定位精准，流量在节假日会暴涨，要求弹性伸缩实例，最大实例数 3 个，期望实例数 2 个，最小实例数 1 个。 
13. 集群内实例 CPU 使用率希望控制在 40%-70%。CPU 使用率超过 70%需要增加实例，小于 40%则缩减实例。 
14. 为了保证业务服务性能，希望更换集群内最早创建的实例 
15. 为了防止数据丢失，需要做前端云主机和数据库进行备份，要求每周日晚 22 点对 opencart 进行备份，每天晚上 22 点对数据库进行备份，并进行还原测试。 
16. 要求将 Linux 云计算架构师第一阶段综合测试阶段 DHCP 服务器（永久关闭 DHCP 服务，防止 DHCP 与云上冲突；卸载光盘，减少迁移数据量）迁移到北京四，并确保数据的完整性。
# 0 实验前准备
## 0 .1 全局网络规划
创建两个 VPC 并分别创建一个子网：
